import { ClockDisplay } from './components/ClockDisplay';
import { UsageChart } from './components/UsageChart';
import { WeeklyHeatmap } from './components/WeeklyHeatmap';
import { PredictionPanel } from './components/PredictionPanel';
import { PromotionHistory } from './components/PromotionHistory';

export default function App() {
  return (
    <div className="min-h-screen bg-[#0a0a0f] p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        {/* Header */}
        <header className="pt-8 pb-4">
          <div className="flex items-baseline gap-3 mb-1">
            <span className="text-[#c4a1ff] font-['JetBrains_Mono'] tracking-wider text-sm uppercase">sys://monitor</span>
            <div className="flex-1 h-px bg-gradient-to-r from-[#c4a1ff]/30 to-transparent" />
          </div>
          <h1 className="text-[#e2e2e8] font-['JetBrains_Mono'] text-3xl md:text-4xl tracking-tight mt-3">
            Claude Code<br />
            <span className="text-[#c4a1ff]">Promotion Clock</span>
          </h1>
          <p className="text-[#6b6b80] mt-3 max-w-lg">
            Real-time usage monitoring with intelligent promotion timing predictions based on historical patterns.
          </p>
        </header>

        {/* Top Row */}
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          <div className="lg:col-span-2">
            <ClockDisplay />
          </div>
          <div className="lg:col-span-3">
            <PredictionPanel />
          </div>
        </div>

        {/* Usage Chart */}
        <UsageChart />

        {/* Weekly Heatmap */}
        <WeeklyHeatmap />

        {/* Promotion History */}
        <PromotionHistory />

        {/* Footer */}
        <footer className="border-t border-white/5 pt-6 pb-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <p className="text-[#6b6b80] text-sm max-w-xl">
            Promotions scheduled during off-peak hours minimize user disruption. 
            The algorithm targets windows with less than 40% usage.
          </p>
          <span className="text-[#c4a1ff]/40 font-['JetBrains_Mono'] text-xs">v1.0.0</span>
        </footer>
      </div>
    </div>
  );
}